<template>
                    ...
                    <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
                        ...
                        <form @submit.prevent="onSubmit">
                            <div class="bg-white px-4 pt-5 pb-4">
                                ...
                                <CustomInput type="textarea" class="mb-2" v-model="unit.full_name" label="Nama Satuan" />
                            </div>
  ...
  <script setup>
    ...
    function onSubmit() {
        loading.value = true
        if(unit.value.id) {
            store.dispatch('updateCategory', unit.value)
                .then(response => {
                    loading.value = false;
            ...
            store.dispatch('createCategory', unit.value)
                .then(response => {
                    loading.value = false;
