...
const termAndCondition = ref({
    id: props.termAndCondition.id,
    title: props.termAndCondition.title,
    content: props.termAndCondition.content,
})
const errors = ref({
    title: [],
    content: [],
...
function onSubmit() {
    ...
    if (termAndCondition.value.id) {
        store.dispatch('updateTermAndCondition', termAndCondition.value)
            .then(response => {
                ...
                if (response.status === 200) {
                    ...
                    store.commit('showToast', 'Term And Condition berhasil diupdate.');
        ...
        store.dispatch('createTermAndCondition', termAndCondition.value)
            .then(response => {
                loading.value = false;
                if (response.status === 201) {
                    // TODO show notification 
                    store.commit('showToast', 'Term And Condition berhasil ditambah.');
                    store.dispatch('getTermAndConditions')
                    closeModal()
                }
            })
