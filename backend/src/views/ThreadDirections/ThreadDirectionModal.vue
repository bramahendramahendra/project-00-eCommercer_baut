<template>
                    ...
                    <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
                        ...
                        <header class="py-3 px-4 flex justify-between items-center">
                            <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                                {{ threadDirection.id ? `Update Thread Direction: "${props.threadDirection.name}"` : 'Tambah Thread Direction Baru' }}
                            </DialogTitle>
  ...
  <script setup>
    ...
    function onSubmit() {
        loading.value = true
        if(threadDirection.value.id) {
            store.dispatch('updateThreadDirection', threadDirection.value)
                .then(response => {
                    loading.value = false;
