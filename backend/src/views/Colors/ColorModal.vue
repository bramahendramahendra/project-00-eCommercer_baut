<template>
                    ...
                    <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
                        ...
                        <header class="py-3 px-4 flex justify-between items-center">
                            <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                                {{ color.id ? `Update Warna: "${props.color.name}"` : 'Tambah Warna Baru' }}
                            </DialogTitle>
                        ...
                        <form @submit.prevent="onSubmit">
                            <div class="bg-white px-4 pt-5 pb-4">
                                <CustomInput class="mb-2" v-model="color.name" label="Nama Warna" />
                            </div>
  ...
  <script setup>
    ...
    function onSubmit() {
        loading.value = true
        if(color.value.id) {
            store.dispatch('updateColor', color.value)
                .then(response => {
                    loading.value = false;
            ...
            store.dispatch('createColor', color.value)
                .then(response => {
                    loading.value = false;
